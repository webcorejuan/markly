// Auth pages: Sign up, Log in — shared split-screen layout.

const AuthLayout = ({ title, subtitle, children, footer }) => (
  <div className="markly-root" style={{ display: 'flex', minHeight: '100%', height: '100%' }}>
    {/* Left: form */}
...
const SignUp = () => (
  <AuthLayout title="Create an account" subtitle="1,000 pages free each month. No card, no trial, no surprises.">
    ...
    <input className="input" defaultValue="jordan@acme.co" style={{ marginBottom: 14 }} />
    <label style={{ display: 'block', fontSize: 12, color: 'var(--text-muted)', fontWeight: 500, marginBottom: 6 }}>Password</label>
    <input className="input" type="password" defaultValue="••••••••••••" style={{ marginBottom: 20 }} />
    ...
    <p style={{ fontSize: 13, color: 'var(--text-muted)', textAlign: 'center', marginTop: 20 }}>
      Already have one? <a style={{ color: 'var(--accent)', cursor: 'pointer' }}>Log in</a>
    </p>
  </AuthLayout>
);
...
// ─── Docs ───────────────────────────────────────────────────
const Docs = () => (
  <div className="markly-root" style={{ display: 'grid', gridTemplateColumns: '220px 1fr 220px', height: '100%', minHeight: '100%' }}>
...
Object.assign(window, { AuthLayout, SignUp, Docs });
