...
import { Icons, StatusDot, Wordmark, Avatar } from './atoms'
...
export default function Landing() {
  ...
  return (
    <div className="markly-root" style={{ width: '100%', overflowX: 'hidden' }}>
      {/* Nav */}
      <nav style={{
        ...
        <div style={{ flex: 1 }} />
        <Link to="/login" style={{ fontSize: 14, color: 'var(--text-body)', cursor: 'pointer', padding: '6px 12px', textDecoration: 'none' }}>Log in</Link>
        <Link to="/signup" className="btn btn-primary">Get an API key <Icons.Arrow size={14} /></Link>
      </nav>
      ...
      <section style={{ padding: '80px 56px 56px', display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 64, alignItems: 'start' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 40 }}>
            <Link to="/signup" className="btn btn-primary" style={{ padding: '12px 20px', fontSize: 15 }}>Get an API key <Icons.Arrow size={14} /></Link>
            <Link to="/docs" className="btn btn-secondary" style={{ padding: '12px 20px', fontSize: 15 }}>Read the docs</Link>
          </div>
      ...
      <section style={{ padding: '24px 56px 88px', background: 'var(--bg-panel)', borderTop: '1px solid var(--border-strong)', borderBottom: '1px solid var(--border-strong)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {[
            { n: 'Free',  p: '$0',      u: '1,000 pages / mo', f: ['Playground', 'One API key', 'Community support'], cta: 'Start free', to: '/signup' },
            { n: 'Pro',   p: '$0.001',  u: 'per page, after 1k free', f: ['Five keys', 'Logs for 30 days', 'Email support'], cta: 'Start Pro', hi: true, to: '/signup' },
            { n: 'Scale', p: 'Volume',  u: 'from $0.0004 / page', f: ['Unlimited keys', 'Private models', 'DPA & SSO'], cta: 'Talk to founder', to: '/signup' },
          ].map(tier => (
            <div key={tier.n} className="card" style={{ padding: 28, background: tier.hi ? 'var(--text-primary)' : 'var(--bg-card)', color: tier.hi ? 'var(--text-inverse)' : undefined, border: tier.hi ? '1px solid var(--text-primary)' : undefined }}>
              ...
              <Link to={tier.to} className={tier.hi ? 'btn' : 'btn btn-secondary'} style={tier.hi ? { background: 'var(--accent)', color: '#fff', width: '100%', justifyContent: 'center', padding: '10px' } : { width: '100%', justifyContent: 'center', padding: '10px' }}>
                {tier.cta}
              </Link>
            </div>
          ))}
